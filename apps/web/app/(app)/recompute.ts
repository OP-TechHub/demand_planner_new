...
export type RecomputeResult = { error: string | null; computedAt?: string; ms?: number };

/**
 * Recompute a plan: load inputs, run the pure engine, and persist the computed
 * tables. Inputs are read and results written with the SERVICE-ROLE client
 * (computed tables are service-role-write-only), but we first verify the caller
 * can actually see the plan under RLS, so a stranger can't recompute it.
 */
export async function recompute(planId: string): Promise<RecomputeResult> {
  ...
  const { data: allowed } = await rls.from('plans').select('id').eq('id', planId).maybeSingle();
  if (!allowed) return { error: 'Plan not found or not accessible.' };
  ...
  // --- map to DB rows (month_index is 1-based) ---
  ...
  const tables = ['plan_rank', 'allocations', 'rolling_results', 'unallocated_wr', 'pipeline_wr', 'plan_summary'];
  for (const t of tables) {
    const { error } = await svc.from(t).delete().eq('plan_id', planId);
    if (error) return { error: `clearing ${t}: ${error.message}` };
  }
  ...
  for (const [t, rows] of inserts) {
    ...
    if (error) return { error: `writing ${t}: ${error.message}` };
  }
  ...
  return { error: null, computedAt, ms: Date.now() - started };
}
