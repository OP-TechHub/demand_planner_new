-- Teach the two plan-data routines about `harvest_request`.
--
-- The request plan is month-indexed and plan-scoped, so it has to move with the
-- rest of the plan's inputs. Without this:
--
--   • roll_plan_forward shifts demand and harvest by N months but leaves the
--     request behind — every requested quantity would silently end up attributed
--     to the WRONG calendar month, which is worse than losing it.
--   • restore_plan_from_snapshot reverts demand and harvest but keeps the
--     current request, leaving the plan half-restored.
--
-- Both are rewritten whole (create or replace) with the request handled the same
-- way as harvest.
...
begin
  ...
  -- The request plan is plan-scoped and keyed only by month, so it copies directly.
  insert into demand_planner.harvest_request (plan_id, month_index, quantity_kg_wr, created_by, updated_by)
